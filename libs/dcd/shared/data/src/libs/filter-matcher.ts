...
export function stateMatchesEquality(inputState: boolean, filterState: string, equality: StateEquality): boolean {
  if (!filterState) {
    return true;
  }
...
export function textArrayMatchesEquality(input: string[], filterValue: string, equality: TextEquality): boolean {
  // If it contains OR, split it up and try again for each piece (one has to be true)
  if (filterValue.indexOf(OR) !== -1) {
    return filterValue.split(OR).some((inputToken) => {
  ...
  const transformedInput =
    input
      ...
      .sort()
      .toString() || '[]';
  const transformedFilterValue = filterValue
    ? filterValue
        .split(',')
        .map((v) => `"${v}"`)
        .sort()
