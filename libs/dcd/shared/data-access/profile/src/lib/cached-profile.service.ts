...
export class CachedProfileService extends ProfileService {
  ...
  private getStorageId(membershipType: number | string, membershipId: string) {
    return `${membershipType}-${membershipId}`;
  }
  private getProfileFromCache(membershipType: number | string, membershipId: string) {
    return this.idbKeyValService.get(this.getStorageId(membershipType, membershipId), this.profileStore);
  }

  getProfile(membershipType: number | string, membershipId: string): Observable<MemberProfile> {
